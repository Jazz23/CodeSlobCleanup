...
The **Code Slob Cleanup** process follows a rigorous 5-phase lifecycle to ensure that code is cleaned effectively and safely.
...
2.  **Exclusions**: The tool reads `code-slob-cleanup.json` (if it exists) to strictly exclude specific paths or functions.
...
3.  **Feedback Loop**: If verification fails, the tool will analyze the failing input and attempt to fix the refactored code to handle the failure, retrying up to 3 times. If it still fails, the refactored code is **discarded**, and the original code remains unchanged.

## Phase 4: Application (Patch)

Once a function passes verification, it is ready to be moved back into the main codebase.
...
- **Brittle Test Fixing**: If an existing test fails because it relied on the internal implementation of the "slob" code, the tool will update the test to be more robust while preserving its original intent. Essentially, we "trust" Hypothesis more than the original test.
